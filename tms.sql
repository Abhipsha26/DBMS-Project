...
5. index.php
SELECT * from tbltourpackages order by rand() limit 4
...
17. signin.php
SELECT EmailId,Password FROM tblusers WHERE EmailId=:email and Password=:password

18. signup.php
INSERT INTO  tblusers(FullName,MobileNumber,EmailId,Password) VALUES(:fname,:mnumber,:email,:password)";

17. writeup.php
INSERT INTO  tblissues(UserEmail,Issue,Description) VALUES(:email,:issue,:description)";


ADMIN PAGES--------------------------
20. change-img.php
update TblTourPackages set PackageImage=:pimage where PackageId=:imgid"
SELECT PackageImage from TblTourPackages where PackageId=:imgid

21. change-password.php
...
SELECT PackageId from tbltourpackages";
SELECT id from tblenquiry";
SELECT id from tblenquiry where (Status is null || Status='')";
SELECT id from tblenquiry where (Status='1')"
SELECT BookingId from tblbooking";
SELECT BookingId from tblbooking where (status is null || status='')";
SELECT BookingId from tblbooking where (status='2')";
SELECT BookingId from tblbooking where (status='1')";

f24. forgot-password.php
SELECT EmailId FROM admin WHERE EmailId=:email and MobileNumber=:mobile";
update admin set Password=:newpassword where EmailId=:email and MobileNumber=:mobile"

25. index.php
SELECT UserName,Password FROM admin WHERE UserName=:uname and Password=:password"

26. logout.php
 27.manage-booking.php
...
28.Manage-queries.php
UPDATE tblenquiry SET Status=:status WHERE  id=:eid";
delete from tblenquiry where id =:id";
SELECT * from tblenquiry";

29. manageissue.php
delete from tblissues where id =:id";
SELECT tblissues.id as id,tblusers.FullName as fname,tblusers.MobileNumber as mnumber,tblusers.EmailId as email,tblissues.Issue as issue,tblissues.Description as Description,tblissues.PostingDate as PostingDate from tblissues left join tblusers on tblusers.EmailId=tblissues.UserEmail";

30. managepackages.php
delete from tbltourpackages where PackageId =:id";
delete from tbltourpackages where PackageId =:id";
SELECT * from tblTourPackages";

31. managepages.php
UPDATE tblpages SET detail=:pagedetails WHERE type=:pagetype";
SELECT detail from tblpages where type=:pagetype";

32. manageuser.phop
SELECT * from tblusers";

33. profile.php
update admin set Name=:name,EmailId=:email,MobileNumber=:mobile where UserName=:adminid";
SELECT * from admin where UserName=:adminid";

34. updateissue.php
UPDATE tblissues SET AdminRemark=:remark WHERE  id=:iid";
SELECT * from tblissues where id=:iid";

35. update-package.php
update TblTourPackages set PackageName=:pname,PackageType=:ptype,PackageLocation=:plocation,PackagePrice=:pprice,PackageFetures=:pfeatures,PackageDetails=:pdetails where PackageId=:pid";
"SELECT * from TblTourPackages where PackageId=:pid";
